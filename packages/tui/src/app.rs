...
impl App {
    ...
    fn handle_key_event(&mut self, key: KeyEvent) {
        ...
        match key.code {
            ...
            KeyCode::Char('k') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                self.mode = Mode::Office;
                self.state.office_state.input_focused = true;
                self.state.library_state.input_focused = false;
                return;
            }
            KeyCode::Char('b') if key.modifiers.contains(KeyModifiers::CONTROL) => {
                self.mode = Mode::Library;
                self.state.office_state.input_focused = false;
                self.state.library_state.input_focused = true;
                return;
            }
            // Ctrl+1..5 — direct mode jump
            ...
            KeyCode::Tab if !self.state.settings_open && !self.state.command_palette_open => {
                self.mode = match self.mode {
                    Mode::Office => Mode::Library,
                    Mode::Library => Mode::Symbolic,
                    Mode::Symbolic => Mode::Losion,
                    Mode::Losion => Mode::Gpu,
                    Mode::Gpu => Mode::Office,
                };
                return;
            }
        ...
        match self.mode {
            Mode::Office => crate::modes::office::handle_key(&mut self.state, key, &self.action_tx),
            Mode::Library => crate::modes::library::handle_key(&mut self.state, key, &self.action_tx),
            Mode::Symbolic | Mode::Losion | Mode::Gpu => {
                // Placeholder modes — no key handling yet
            }
        }
    }
    ...
    fn handle_mouse_event(&mut self, mouse: crossterm::event::MouseEvent) {
        ...
        match mouse.kind {
            MouseEventKind::ScrollUp => {
                match self.mode {
                    Mode::Office => self.state.office_state.scroll_up(),
            ...
            MouseEventKind::ScrollDown => {
                match self.mode {
                    Mode::Office => self.state.office_state.scroll_down(),
        ...
        match key.code {
            ...
            KeyCode::Down => {
                let max = match self.state.settings_tab {
                    crate::state::SettingsTab::Workers => self.state.office_state.workers_list.len().saturating_sub(1),
    ...
    fn execute_command(&mut self, action: String) {
        match action.as_str() {
            "switch_office" => self.mode = Mode::Office,
            "switch_library" => self.mode = Mode::Library,
            "switch_symbolic" => self.mode = Mode::Symbolic,
            "switch_losion" => self.mode = Mode::Losion,
            "switch_gpu" => self.mode = Mode::Gpu,
            "toggle_theme" => self.cycle_theme(),
            "open_settings" => self.state.settings_open = true,
            "clear_chat" => self.state.office_state.manager_messages.clear(),
            "focus_mode" => self.state.office_state.focus_mode = !self.state.office_state.focus_mode,
    ...
    fn render(&self, f: &mut Frame) {
        let area = f.area();
        match self.mode {
            Mode::Office => crate::modes::office::render(f, area, &self.state, &self.theme, self.tick),
            Mode::Library => crate::modes::library::render(f, area, &self.state, &self.theme, self.tick),
            Mode::Symbolic => crate::modes::symbolic::render(f, area, &self.state, &self.theme),
            Mode::Losion => crate::modes::losion::render(f, area, &self.state, &self.theme),
            Mode::Gpu => crate::modes::gpu::render(f, area, &self.state, &self.theme),
        }
