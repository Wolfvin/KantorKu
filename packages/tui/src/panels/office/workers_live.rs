...
use crate::state::office_state::{OfficeState, WorkersTab};
use crate::ui::components::{squad_color, spinner_char, phase_label, worker_status_icon};
use crate::ui::theme::Theme;

/// Render the tabbed Workers Live panel (middle column in Office mode)
pub fn render(f: &mut Frame, area: Rect, state: &OfficeState, theme: &Theme, tick: u64) {
    ...
    match state.active_tab {
        ...
        WorkersTab::Briefing => crate::panels::office::briefing::render(f, chunks[2], state, theme),
        WorkersTab::Dag => crate::panels::office::dag::render(f, chunks[2], state, theme),
        WorkersTab::Events => crate::panels::office::events::render(f, chunks[2], state, theme),
    }
}

fn render_workers_tab(f: &mut Frame, area: Rect, state: &OfficeState, theme: &Theme, tick: u64) {
    ...
    let events: Vec<&crate::state::office_state::WorkerEvent> = state
        .worker_events
        .iter()
        .rev()
        .take(visible_count)
