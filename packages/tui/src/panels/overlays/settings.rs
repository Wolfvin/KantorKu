...
fn render_workers_tab(f: &mut Frame, area: Rect, state: &AppState, theme: &Theme) {
    let items: Vec<ListItem> = state.office_state.workers_list.iter().enumerate().map(|(i, worker_id)| {
    ...
    let count = state.office_state.workers_list.len();
...
fn render_keybindings_tab(f: &mut Frame, area: Rect, theme: &Theme) {
    let bindings = vec![
        ("Tab", "Switch mode (Office ↔ Library)"),
        ("Ctrl+K", "Switch to Office"),
