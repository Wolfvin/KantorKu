...
from kantorku.losion.config import LosionConfig, RecurrentConfig, JEPAConfig
from kantorku.losion.models.shared import RMSNorm as _SharedRMSNorm, WeightInitMixin
...
def _build_ssm(config: LosionConfig) -> nn.Module:
    ...
    try:
        ...
        if ssm_cfg.use_structured_sparse:
            from kantorku.losion.core.ssm.structured_sparse import StructuredSparseSSM, StructuredSparseSSMConfig
        ...
        if ssm_cfg.use_routing_mamba:
            from kantorku.losion.core.ssm.routing_mamba import RoutingMamba, RoutingMambaConfig
        ...
        if ssm_cfg.use_mamba3:
            from kantorku.losion.core.ssm.mamba3 import Mamba3SSD
            return Mamba3SSD(
                d_model=d_model,
        ...
        if ssm_cfg.use_liquid:
            from kantorku.losion.core.ssm.liquid_ssm import LiquidSSMTerpaduLayer
            return LiquidSSMTerpaduLayer(config)

        # Default: Mamba-2 SSD
        from kantorku.losion.core.ssm.mamba2 import Mamba2SSD
...
def _build_attention(config: LosionConfig) -> nn.Module:
    ...
    try:
        ...
        if config.child_3w.enabled:
            from kantorku.losion.core.attention.child_3w import Child3WAttention, Child3WConfig as _Child3WCfg
            c3w_cfg = _Child3WCfg(
                d_model=d_model,
        ...
        if attn_cfg.use_moba:
            from kantorku.losion.core.attention.moba import MoBAAttention, MoBAConfig
            moba_cfg = MoBAConfig(
        ...
        if attn_cfg.use_gated_attention:
            from kantorku.losion.core.attention.gated_attention import GatedMultiHeadAttention, GatedAttentionConfig
        ...
        if attn_cfg.use_lightning:
            from kantorku.losion.core.attention.lightning_attention import LightningAttention
            return LightningAttention(d_model=d_model, n_heads=attn_cfg.n_heads, d_kv=attn_cfg.d_kv,
                                      mla_latent_dim=attn_cfg.mla_latent_dim)

        # Default: KDA+MLA
        from kantorku.losion.core.attention.kda_mla import KDAMLA
...
def _build_moe(config: LosionConfig) -> nn.Module:
    ...
    try:
        ...
        if ret_cfg.use_infinite_moe:
            from kantorku.losion.core.retrieval.infinite_moe import InfiniteMoE, InfiniteMoEConfig
        ...
        if ret_cfg.use_smore:
            from kantorku.losion.core.retrieval.smore import SmoreMoE, SmoreConfig
        ...
        if ret_cfg.use_symbolic_moe:
            from kantorku.losion.core.retrieval.symbolic_moe import SymbolicMoERouter
            ...
            from kantorku.losion.core.retrieval.aux_free_moe import AuxFreeMoE
            base_moe = AuxFreeMoE(
                d_model=d_model,
                d_ff=d_ff,
                num_experts=num_experts,
...
def _build_router(config: LosionConfig) -> nn.Module:
    ...
    """
    try:
        from kantorku.losion.core.router.router import AdaptiveRouter
...
class LosionModelV2(nn.Module, WeightInitMixin):
    ...
    def __init__(self, config: LosionConfig) -> None:
        ...
        if self.use_attn_res:
            try:
                from kantorku.losion.core.attention.attn_res import AttnResConfig as _AttnResCfg, AttnResManager
        ...
        if self.use_evoformer:
            try:
                from kantorku.losion.core.feedback.evoformer import EvoformerConfig as _EvoCfg, EvoformerManager
                _evo_cfg = _EvoCfg(
                    d_model=config.d_model,
        ...
        if self.use_dual_memory:
            try:
                from kantorku.losion.core.memory.dual_memory import DualMemoryConfig as _DMCfg, DualMemorySystem
                _dm_cfg = _DMCfg(
                    d_model=config.d_model,
        ...
        if self.use_rdt:
            try:
                from kantorku.losion.core.recurrent.rdt import RecurrentDepthBlock
