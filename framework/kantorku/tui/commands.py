...
# ═══════════════════════════════════════════════════════════════════
# System — CLI commands now available as slash commands inside TUI
# ═══════════════════════════════════════════════════════════════════

@command("serve", "Start the API server", "/serve [--host HOST] [--port PORT]")
async def cmd_serve(tui: Any, args: str) -> str:
    """Start the kantorku API server in background.

    Usage:
        /serve                      Start on 0.0.0.0:8000
        /serve --port 9000          Use custom port
    ...
    while i < len(parts):
        if parts[i] == "--host" and i + 1 < len(parts):
            ...
            try:
                ...
                return f"[red]Invalid port: {parts[i + 1]}[/red]"
            i += 2
        else:
            i += 1
    ...
    try:
        ...
        # Run server in background thread so TUI stays responsive
        import threading
        ...
        def _run_server():
            try:
                ...
                pass  # Server errors logged by uvicorn

        server_thread = threading.Thread(target=_run_server, daemon=True)
        server_thread.start()

        return (
            f"[green]API server starting on {host}:{port}[/green]\n"
            f"[dim]Running in background (daemon thread)[/dim]\n"
            f"[dim]Connect from another terminal: kantorku --remote --url http://{host}:{port}[/dim]"
        )
    except ImportError as e:
        return f"[red]Server dependencies not installed: {e}[/red]"
    except Exception as e:
        return f"[red]Failed to start server: {e}[/red]"


@command("init", "Scaffold a new kantorku project", "/init [name]")
async def cmd_init(tui: Any, args: str) -> str:
    """Scaffold a new kantorku project directory.

    Usage:
        /init           Create 'my-office' project in current directory
        /init my-proj   Create 'my-proj' project
    """
    from pathlib import Path

    project_name = args.strip() or "my-office"
...
'''
    ...
    return (
        f"[green]Created kantorku project in ./{project_dir}/[/green]\n"
        f"  {project_dir}/kantorku.toml  (configuration)\n"
        f"  {project_dir}/data/          (memory storage)\n"
        f"  {project_dir}/workers/       (custom workers)\n"
        f"\n"
        f"[dim]Next: edit kantorku.toml, then restart kantorku with -c {project_dir}/kantorku.toml[/dim]"
    )


@command("version", "Show kantorku version", "/version")
async def cmd_version(tui: Any, args: str) -> str:
    """Show kantorku version and runtime info."""
    try:
    ...
    lines = [
        f"[bold cyan]kantorku v{__version__}[/bold cyan]",
    ...
    if _embedded(tui):
        lines.append(f"  Mode:      Embedded (in-process)")
    elif _remote(tui):
        lines.append(f"  Mode:      Remote ({tui.server_url})")
    else:
        lines.append(f"  Mode:      Not connected")
